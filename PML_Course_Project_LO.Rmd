...
```
With random forests as model and the same training parameters as for modFit_60 the OOB estimate of error rate increased from 0.9 to 1.2. The running time (training) was more than halved.

####<br/>Testing various models using the 20 most influential predictors####
In addition to random forests (rf) other models were tested with the training data consisting of the twenty most important varioables. The purpose was to investigate the difference in misclassification rates for the models. To reduce running time the number two was used for K-fold cv. Models tested were Generalized Boosted Model (gbm), Naive Bayes (nb),  Regularized Discriminant Analysis (rda) and finally Linear Discriminant Analysis (lda). The error rate on the testing set increased in the order the models are mentioned. The  gbm model, the most accurate of the alternative studied models, showed a misclassification error of 5.1% where rf achieved 1.3%. This is the main reason for choosing random forest as the final model in this project.
